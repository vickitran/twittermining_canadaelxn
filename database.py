...
USE dbname;
CREATE TABLE tables (Username VARCHAR(20), Tweet VARCHAR(140), Time INT(140), NDP INT(13), Conservative INT(13), Liberal INT(13)); 
'''
...
c.execute("CREATE TABLE IF NOT EXISTS tweetTable (username VARCHAR(20), tweet VARCHAR(1000), time INT(140), ndp INT(13), conservative INT(13), liberal INT(13))")
conn.commit()
...
def insertRow(user, tweet, time, freqNDP, freqCons, freqLib):
	...
	c.execute("REPLACE tweetTable (username, tweet, time, ndp, conservative, liberal) VALUES (%s,%s,%s,%s,%s,%s)",
